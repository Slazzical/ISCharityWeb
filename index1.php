...
<head>
    <meta charset="utf-8">
    <title>Donation System - Charity Owner Module</title>
...
function printData2()
{
   ...
   var delbtn=document.getElementById("delbtn1");
   delbtn.style.display = 'none';
   newWin= window.open("");
   newWin.document.write(divToPrint.outerHTML);
   newWin.print();
...
<?php
$sql = "SELECT `charities`.`Charity_ID`, `charities`.`User_ID`, `charities`.`Location`, `charities`.`Long`, `charities`.`Lat`, `charities`.`Name`, `charities`.`Description`, `charities`.`Image`, `users`.`Fullname`, `users`.`Phone_Number`, `users`.`Email_Address` FROM `charities` JOIN `users` ON `charities`.`User_ID` = `users`.`User_ID` WHERE `charities`.`User_ID` = '$filter'";
...
<td><button id="delbtn" class="btn btn-primary py-3 px-5" onclick="return confirm('Are you sure that you want to delete this charity?')?window.location.href='php/functions.php?action=deleteC&id=<?php echo($row["Charity_ID"]); ?>':true;" title='Delete Charity Center'>Delete</button></td>
</tr>
<?php
}
...
$sql = "
    ...
    FROM donations 
    JOIN charities ON donations.Charity_ID = charities.Charity_ID
...
<br>
    ...
    <!-- Database End -->
        ...
        <!-- My Module Start -->
    <div class="container-fluid bg-registration py-5" style="margin: 90px 0;" id="mod">
        <div class="container py-5">
            <div class="row align-items-center">
<div class="container">
    <div class="row">
        <div class="col-lg-12">
            <div class="card border-0">
                <div class="row">
                    <div class="col-md-6">
                      <br>
                        <iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d255282.32335424496!2d36.84739685!3d-1.3032089500000001!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x182f1172d84d49a7%3A0xf7cf0254b297924c!2sNairobi!5e0!3m2!1sen!2ske!4v1721176531961!5m2!1sen!2ske"
                            width="600" height="700" style="border:0;" allowfullscreen="" loading="lazy"
                            referrerpolicy="no-referrer-when-downgrade"></iframe>
                    </div>
                    <div class="col-md-6">
                        <div class="card-header bg-light text-center p-4">
                            <h1 class="m-0">Add A Charity Center</h1>
                        </div>
                        <div class="card-body rounded-bottom bg-primary p-5">
                            <form action="php/functions.php" method="POST" enctype="multipart/form-data">
                                <div class="form-group">
                                    <input type="text" class="form-control border-0 p-4" name="cname"
                                        placeholder="Charity Center Name" required="required" />
                                </div>
                                <div class="form-group">
                                    <input type="text" class="form-control border-0 p-4" name="loc"
                                        placeholder="Charity Center Location" required="required" />
                                </div>
                                <div class="form-group">
                                    <input type="text" name="lat"
                                        class="form-control border-0 p-4" placeholder="Charity Center Latitude"
                                        required="required" />
                                </div>   
                                <div class="form-group">
                                    <input type="text" class="form-control border-0 p-4" name="long"
                                        placeholder="Charity Center Longitude" required="required" />
                                </div>
                                <div class="form-group">
                                    <label style="color: white;">Charity Center Image:</label>
                                    <br>
                                    <input type="file" name="image" accept=".jpg, .png, .jpeg"
                                        class="form-control border-0 p-4" required="required" />
                                </div>                                                              
                                <div class="form-group">
                                    <textarea type="text" rows="5" name="desc" class="form-control border-0 p-4"
                                        placeholder="Charity Center Description" required="required" /></textarea>
                                </div>
                                <div>
                                    <button class="btn btn-dark btn-block border-0 py-3" type="submit"
                                        name="addC">Add</button>
                                </div>
                            </form>
                        </div>
                    </div>
                    ...
                    <div class="card border-0">
                        <div class="card-header bg-light text-center p-4">
                            <h1 class="m-0">Send A Message</h1>
                        </div>
                        <div class="card-body rounded-bottom bg-primary p-5">
                            <form action="php/functions.php" method="POST" enctype="multipart/form-data">
                                <div class="form-group">
                                    <select class="custom-select border-0 px-4" style="height: 47px;" required name="did">
                                        <option selected disabled value="">Select A Donation ID</option>
                <?php
                ...
                ?>
                <option value="<?php echo $category["Donation_ID"];?>"><?php echo $category["Donation_ID"];?></option>
                <?php
                endwhile;
                ?>
                                    </select>
                                                                ...
                                                                <div class="form-group">
                                    <textarea type="text" name="mes" class="form-control border-0 p-4"
                                        placeholder="Message Donor..." required="required" /></textarea>
                                </div>
                                <div>
                                    <button class="btn btn-dark btn-block border-0 py-3" type="submit" name="sendM">Send Message</button>
                                </div>
                            </form>
                        </div>
                    </div>
